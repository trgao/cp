// Geometry 2D from code library on github
#include<bits/stdc++.h>
using namespace std;
using ll = long long;
...
struct line {
    ...
    ll side(PT p) { return sign(cross(v, p) - c); }
...
ll is_parallel(PT a, PT b, PT c, PT d) {
...
ll point_line_relation(PT a, PT b, PT p) {
    ll c = sign(cross(p - a, b - a));
    if (c < 0) return 1;
    if (c > 0) return 2;
    return 3;
}
...
ll seg_line_relation(PT a, PT b, PT c, PT d) {
...
ll circle_point_relation(PT p, double r, PT b) {
    double d = dist(p, b);
...
// 0 if outside, 1 if on circumference, 2 if inside circle
ll circle_line_relation(PT p, double r, PT a, PT b) {
...
ll circle_circle_relation(PT a, double r, PT b, double R) {
    double d = dist(a, b);
...
ll get_circle(PT a, PT b, double r, circle &c1, circle &c2) {
...
ll get_circle(line u, PT q, double r1, circle &c1, circle &c2) {
...
ll tangent_lines_from_point(PT p, double r, PT q, line &u, line &v) {
    ll x = sign(dist2(p, q) - r * r);
...
ll tangents_lines_from_circle(PT c1, double r1, PT c2, double r2, bool inner, line &u, line &v) {
    if (inner) r2 = -r2;
    PT d = c2 - c1;
...
struct CircleUnion {
    ...
    double arc, pol;
    inline ll sign(double x) {return x < -eps ? -1 : x > eps;}
    inline ll sign(double x, double y) {return sign(x - y);}
    ...
    void getarea(ll i, double lef, double rig) {
...
ll is_point_in_triangle(PT a, PT b, PT c, PT p) {
    if (sign(cross(b - a,c - a)) < 0) swap(b, c);
    ll c1 = sign(cross(b - a,p - a));
    ll c2 = sign(cross(c - b,p - b));
    ll c3 = sign(cross(a - c,p - c));
    if (c1<0 || c2<0 || c3 < 0) return 1;
    if (c1 + c2 + c3 != 3) return 0;
    return -1;
}
double perimeter(vector<PT> &p) {
    double ans=0; ll n = p.size();
    for (ll i = 0; i < n; i++) ans += dist(p[i], p[(i + 1) % n]);
    return ans;
}
double area(vector<PT> &p) {
    double ans = 0; ll n = p.size();
    for (ll i = 0; i < n; i++) ans += cross(p[i], p[(i + 1) % n]);
    return fabs(ans) * 0.5;
}
...
PT centroid(vector<PT> &p) {
    ll n = p.size(); PT c(0, 0);
    double sum = 0;
    for (ll i = 0; i < n; i++) sum += cross(p[i], p[(i + 1) % n]);
...
PT geometric_median(vector<PT> p) {
	auto tot_dist = [&](PT z) {
	    double res = 0;
	    for (ll i = 0; i < p.size(); i++) res += dist(p[i], z);
	    return res;
	};
	auto findY = [&](double x) {
...
ll winding_number(vector<PT> &p, const PT& z) { // O(n)
...
// -1 if strictly inside, 0 if on the polygon, 1 if strictly outside
ll is_point_in_polygon(vector<PT> &p, const PT& z) { // O(n)
    ll k = winding_number(p, z);
...
ll extreme_vertex(vector<PT> &p, const PT &z, const ll top) { // O(log n)
    ll n = p.size();
    if (n == 1) return 0;
	double ans = dot(p[0], z); ll id = 0;
...
double minimum_enclosing_rectangle(vector<PT> &p) {
	...
	ll mndot = 0; double tmp = dot(p[1] - p[0], p[0]);
	for (ll i = 1; i < n; i++) {
...
double polygon_line_intersection(vector<PT> p, PT a, PT b) {
    ...
    for (ll i = 0; i < n; i++) {
        ll s1 = orientation(a, b, p[i]);
        ll s2 = orientation(a, b, p[i + 1]);
        if (s1 == s2) continue;
    ...
    for (ll i = 0, j = 0; i + 1 < (ll)vec.size(); i++){
        j += vec[i].second;
...
array<ll, 2> convex_line_intersection(vector<PT> &p, PT a, PT b, ll top) {
	ll end_a = extreme_vertex(p, (a - b).perp(), top);
    ll end_b = extreme_vertex(p, (b - a).perp(), top);
    auto cmp_l = [&](ll i) { return orientation(a, p[i], b); };
    if (cmp_l(end_a) < 0 || cmp_l(end_b) > 0)
        return {-1, -1}; // no intersection
    array<ll, 2> res;
    for (ll i = 0; i < 2; i++) {
        ll lo = end_b, hi = end_a, n = p.size();
        while ((lo + 1) % n != hi) {
    ...
    if (!cmp_l(res[0]) && !cmp_l(res[1])) 
        switch ((res[0] - res[1] + (ll)p.size() + 1) % p.size()) {
...
pair<PT, ll> point_poly_tangent(vector<PT> &p, PT Q, ll dir, ll l, ll r) {
    while (r - l > 1) {
        ll mid = (l + r) >> 1;
    ...
    pair<PT, ll> ret = {p[l], l};
    for (ll i = l + 1; i <= r; i++) ret = orientation(Q, ret.first, p[i]) != dir ? make_pair(p[i], i) : ret;
    return ret;
}
...
pair<ll, ll> tangents_from_point_to_polygon(vector<PT> &p, PT Q){
    ll ccw = point_poly_tangent(p, Q, 1, 0, (ll)p.size() - 1).second;
    ll cw = point_poly_tangent(p, Q, -1, 0, (ll)p.size() - 1).second;
...
double dist_from_point_to_polygon(vector<PT> &p, PT z) {
    ...
    ll n = p.size();
    if (n <= 3) {
        for(ll i = 0; i < n; i++) ans = min(ans, dist_from_point_to_seg(p[i], p[(i + 1) % n], z));
        return ans;
    }
...
// top - upper right vertex
double dist_from_polygon_to_line(vector<PT> &p, PT a, PT b, ll top) { //O(log n)
	PT orth = (b - a).perp();
	if (orientation(a, b, p[0]) > 0) orth = (a - b).perp();
	ll id = extreme_vertex(p, orth, top);
...
double maximum_dist_from_polygon_to_polygon(vector<PT> &u, vector<PT> &v){ //O(n)
    ll n = (ll)u.size(), m = (ll)v.size();
    double ans = 0;
    if (n < 3 || m < 3) {
        for (ll i = 0; i < n; i++) {
            for (ll j = 0; j < m; j++) ans = max(ans, dist2(u[i], v[j]));
        }
        return sqrt(ans);
    }
    if (u[0].x > v[0].x) swap(n, m), swap(u, v);
    ll i = 0, j = 0, step = n + m + 10;
...
struct HP {
    ...
    ll operator < (const HP& rhs) const {
        PT p = b - a;
        PT q = rhs.b - rhs.a;
        ll fp = (p.y < 0 || (p.y == 0 && p.x < 0));
        ll fq = (q.y < 0 || (q.y == 0 && q.x < 0));
...
vector<PT> half_plane_intersection(vector<HP> h) {
    ...
    vector<HP> res; 
    for (ll i = qh; i < qe; i++) res.push_back(q[i]);
    vector<PT> hull;
    if (res.size() > 2) {
        for (ll i = 0; i < res.size(); i++) {
             hull.push_back(res[i].intersection(res[(i + 1) % ((ll)res.size())]));
        }
    }
    return hull;
}
...
double polygon_circle_intersection(vector<PT> &v, PT p, double r) {
    ...
    PT org = {0, 0};
    for(ll i = 0; i < n; i++) {
        ll x = orientation(p, v[i], v[(i + 1) % n]);
        if(x == 0) continue;
...
double maximum_circle_cover(vector<PT> p, double r, circle &c) {
    ...
    for (ll i = 0; i < n; ++i) {
        ...
        vector<pair<double, ll>> events = {{-PI, +1}, {PI, -1}};
        for (ll j = 0; j < n; ++j) {
            if (j == i) continue;
...
vector<vector<PT>> triangulate(vector<PT> p) {
  ...
  while (p.size() >= 3) {
    for (ll i = 0, n = p.size(); i < n; i++) {
      ll pre = i == 0 ? n - 1 : i - 1;;
      ll nxt = i == n - 1 ? 0 : i + 1;;
      ll ori = orientation(p[i], p[pre], p[nxt]);
      if (ori < 0) {
        ll ok = 1;
        for (ll j = 0; j < n; j++) {
...
struct star {
	ll n;    // number of sides of the star
	double r; // radius of the circumcircle
	star(ll _n, double _r) {
		n = _n;
		r = _r;
	}
